...
function hostconvert(url) {
	...
	if(url_host=="bilibili.kankanews.com"){
		url = url.replace(url_host, "www.bilibili.tv");
	}
	if(url_host && cur_host != url_host && url_host != '222.73.227.67') {
...
function checkurl(obj){
	...
	url="book.php?mod=ajax&do=checkurl"+"&url="+encodeURIComponent(url.substring(7));
	window.open(url);
	return false;
}

function showtag(type,id,showid){
	var x = new Ajax('HTML');
	x.get('book.php?mod=ajax&do=loadtag&type='+type+'&id='+id+'&inajax=1', function(s){
		var result=trim(s);
		if(result){
			$(showid).innerHTML=result;
...
function sendchangeemail(){
	...
	var x = new Ajax('HTML');
	x.get('home.php?mod=misc&ac=sendoldemail&inajax=1', function(s){
		var result=trim(s);
		if(!result){return false;}
		$('sendoldemail_con').innerHTML=result;
...
function openvideo(obj){
	//引证
	var thevid=obj.getAttribute('vid');
	var x = new Ajax('HTML');
	x.get('loadvideo.php?type=loadvideo&vid='+thevid, function(s){
		var result=trim(s);
		if(!result){return false;}
...
function cardShow(obj) {
	...
	USERCARDST = setTimeout(function() {ajaxmenu(obj, 500, 1, 2, pos, null, 'p_pop card');}, 100);
}
...
function tojoingroup(fid){
	var thenum='';

	if(typeof($('group_a'))!='undefined'){
		var group_a=$('group_a');
		if(group_a && (typeof(group_a.value)=='string' || typeof(group_a.value)=='number'))thenum=group_a.value;
	}
	var theulr='forum.php?mod=group&action=join&fid='+fid+'&a='+thenum
	location.href=theulr;
}
...
function submitrate(){
	...
	ajaxpost('rateform', 'return_rate', 'return_rate', 'onerror',$('ratesubmit'));
	return false;
}
